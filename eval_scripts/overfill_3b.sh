...
# Set STUDENT_MODEL based on MODEL_PATH
if [ "$MODEL_PATH" == "None" ]; then
    STUDENT_MODEL=friendshipkim/overfill-Llama-3.1-8B-Instruct-pruned-h0.43-i0.43-a0.0-d0.0
else
    STUDENT_MODEL=friendshipkim/Llama-3.1-8B-Instruct-pruned-h0.43-i0.43-a0.0-d0.0
fi
...
echo "=> evaluating Overfill 3b model on ${TASK} with ${N_SHOT} shots"
python overfill/eval/overfill_model_separate.py \
    --model custom_hf \
    --model_args model_path=$MODEL_PATH,teacher_model=$TEACHER_MODEL,student_model=$STUDENT_MODEL \
    --tasks $TASK \
    --num_fewshot $N_SHOT \
    --device cuda \
    ...
    --output_path eval_results/overfill_3b_${TASK}_${N_SHOT}shots