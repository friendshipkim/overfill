...
if [ "$MODEL_PATH" == "None" ]; then
    STUDENT_MODEL=friendshipkim/overfill-Llama-3.2-3B-Instruct-pruned-h0.45-i0.45-a0.0-d0.0
else
    STUDENT_MODEL=friendshipkim/Llama-3.2-3B-Instruct-pruned-h0.45-i0.45-a0.0-d0.0
fi
...
echo "=> evaluating Overfill 1b model on ${TASK} with ${N_SHOT} shots"
python overfill/eval/overfill_model_separate.py \
    ...
    --output_path eval_results/overfill_1b_${TASK}_${N_SHOT}shots