...
echo "=> evaluating pruned 3b model on ${TASK} with ${N_SHOT} shots"
python overfill/eval/prune_model.py \
    --model custom_hf \
    --model_args pretrained=$MODEL_PATH,teacher_model=$TEACHER_MODEL \
    --tasks $TASK \
    --num_fewshot $N_SHOT \
    --device cuda \
    ...
    --output_path eval_results/pruned_3b_${TASK}_${N_SHOT}shots
